...
Modified 2017 by Luther Tychonievich to add $(D remove) functionality

License: Distributed under the Boost Software License, Version 1.0.
(See accompanying file LICENSE_1_0.txt or copy at $(HTTP
boost.org/LICENSE_1_0.txt)).

Authors: $(HTTP erdani.com, Andrei Alexandrescu) $(HTTP www.cs.virginia.edu/luther, Luther Tychonievich)
*/
module std.container.binaryheap2;
...
/**
Performs a linear search to find an element == to value and, if
found, removes it from the store.  Returns whether the element was
found and removed.
 */
    bool remove(ElementType!Store value)
    {
        size_t index = 0;
        while(index < _length && _store[index] != value) index += 1;
        if (index == _length) return false;
        // because binary heaps are complete but this value might not percolate down to the last index,
        // must first percolate it up as if it had the smallest priority
        // and then do a removeFront

        // this loop is copied (modulo variable renaming and without the comparions) from the last part of std.algorithms.sorting HeapOps.percolate
        for (auto child = index; child > 0; child = index)
        {
            index = (child - 1) / 2;
            _store.swapAt(index, child);
        }
        
        // now that the bad node is at the top, remove it
        removeFront;
